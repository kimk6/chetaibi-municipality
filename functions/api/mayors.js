...
export async function onRequestPost(context) {
    ...
    try {
        const { name, period, image_url, name_fr, period_fr, is_dec } = await context.request.json();
        if (!name || !period) return createResponse({ success: false, error: 'الاسم والفترة مطلوبان' }, 400);
        const result = await context.env.DB
            .prepare('INSERT INTO mayors (name,period,image_url,name_fr,period_fr,is_dec) VALUES (?,?,?,?,?,?)')
            .bind(name, period, image_url||'', name_fr||'', period_fr||'', is_dec?1:0)
            .run();
...
export async function onRequestPut(context) {
    ...
    try {
        ...
        await context.env.DB
            .prepare('UPDATE mayors SET name=?,period=?,image_url=?,name_fr=?,period_fr=?,is_dec=? WHERE id=?')
            .bind(name, period, image_url||'', name_fr||'', period_fr||'', is_dec?1:0, id)
            .run();
