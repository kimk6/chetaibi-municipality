// functions/api/beaches.js — v2 مع ألبومات
...
export async function onRequestPost(context) {
    ...
    try {
        ...
        const r = await context.env.DB
            ...
            .bind(name, description || '', image_url || '', is_supervised ?? 1, season || 'صيف').run();
        return createResponse({ success: true, id: r.meta.last_row_id }, 201);
    } catch (e) { return createResponse({ success: false, error: e.message }, 500); }
}
...
export async function onRequestPut(context) {
    ...
    try {
        ...
        await context.env.DB
            ...
            .bind(name, description || '', image_url || '', is_supervised ?? 1, season || 'صيف', id).run();
