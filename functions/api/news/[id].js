            ...
            if (albums.length > 0) {
                ...
                <div class="mt-8">
                    <h3 class="text-lg font-extrabold text-gray-800 mb-4 flex items-center gap-2">
                        <span class="iconify text-emerald-500" data-icon="lucide:images"></span> معرض الصور (${albums.length})
                    </h3>
                    <div class="grid grid-cols-2 md:grid-cols-3 gap-3">
                        ${albums.map(u => `
                        <div class="rounded-xl overflow-hidden shadow cursor-pointer" onclick="openLB('${u}')">
...
<body class="bg-gray-50 text-gray-800">
    ...
    <div class="bg-white border-b border-emerald-100 py-4 shadow-sm">
        <div class="max-w-4xl mx-auto px-4 flex items-center justify-center gap-3">
            <div class="w-14 h-14 rounded-full overflow-hidden shadow-md flex-shrink-0">
                <img src="https://cdn.jsdelivr.net/gh/kimk6/chetaibi-assets-v1@main/ui/logo.webp" class="w-full h-full object-cover">
            </div>
            <div class="text-center">
                <h2 class="text-xl font-extrabold text-emerald-800">بلدية شطايبي</h2>
                <p class="text-xs text-emerald-600">ولاية عنابة - الجزائر</p>
            </div>
        </div>
    </div>
    ...
    <main class="max-w-4xl mx-auto px-4 py-12">
        ...
        <article class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
            ${news.image_url
                ...
                : `<div class="h-40 bg-gradient-to-bl from-emerald-500 to-emerald-700 flex items-center justify-center"><span class="iconify text-white text-7xl" data-icon="lucide:newspaper"></span></div>`
            }
            <div class="p-6 md:p-10">
                <div class="flex items-center gap-3 mb-4 flex-wrap">
                    <span class="inline-block ${cat.bg} text-xs font-bold px-3 py-1 rounded-full">${displayLabel}</span>
                    ${news.is_pinned ? '<span class="text-xs bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full border border-emerald-200 font-bold">📌 مثبت</span>' : ''}
                ...
                <div class="text-gray-600 leading-loose whitespace-pre-wrap">${news.content}</div>
                ${albumHtml}
            </div>
        </article>
    </main>
    ...
    <div id="lb" class="fixed inset-0 bg-black/90 z-50 hidden items-center justify-center p-4" onclick="closeLB()">
        ...
        <button class="absolute top-4 right-4 text-white text-2xl bg-black/50 rounded-full w-10 h-10 flex items-center justify-center font-bold">✕</button>
    </div>
    ...
    <script>
        ...
        function openLB(src){document.getElementById('lb-img').src=src;const l=document.getElementById('lb');l.classList.remove('hidden');l.classList.add('flex');}
        function closeLB(){const l=document.getElementById('lb');l.classList.add('hidden');l.classList.remove('flex');}
    <\/script>
</body>
</html>`;
